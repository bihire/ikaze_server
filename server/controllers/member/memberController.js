...
module.exports = {
  async registerMember(req, res) {
    ...
    try {
      const {
        user_name,first_name, last_name, email, password,approved,phone, memberShip_id
      } = req.body;
      const member = await Member.create({
       user_name, first_name, last_name, email, password, approved, phone, memberShip_id
      }, {
        fields: ["user_name", "email", "password", "first_name", "approved", "last_name", "phone", "memberShip_id" ]
      });
      res.send({
        message: 'it passed',
        data: member,
  ...
  async update(req, res) {
    ...
    let {
      userName, email, password, balance, approved, shopNumber
    } = req.body;
    try {
